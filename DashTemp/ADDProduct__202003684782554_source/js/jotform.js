...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
            ...
            switch (type) {
                ...
                case 'inline':
                    ...
                    if (data.indexOf('|') > -1) {
                        ...
                        var valArr = [];
                        combinedObject = {};
                        $$("#id_" + qid + ' input[id*=' + fieldID + ']').each(function (el) {
                ...
                case 'time':
                    ...
                    var valArr = [];
                    combinedObject = {};
                    var timeElements = JotForm.newDefaultTheme ? "#id_" + data + " select, #id_" + data + " input:not(.form-textbox)" : "#id_" + data + " select";
        ...
        switch (resultFieldType) {
            case "inline":
                if (result.indexOf('|') > -1) {
                    ...
                    var qid = splitResult[0];
                    var fieldId = splitResult[1];
                    var resultFields = $$('#id_' + qid + ' input[id*=' + fieldId + ']').filter(function(el) {return JotForm.isVisible(el);});
                    var hasCombinedField = false;
                    Object.keys(combinedObject).forEach(function(id) {
                        if ($$('#id_' + qid + ' input[id*=' + fieldId + '-' + id + ']')[0]) {
                            hasCombinedField = true;
                            $$('#id_' + qid + ' input[id*=' + fieldId + '-' + id + ']')[0].value = combinedObject[id];
                        }
                    });
    ...
    */
    ...
    checkPwa: function() {
        if (window.location.href.indexOf('?jotform_pwa=1&pwa_id=') > -1) {
            ...
            form.insert(new Element('input', {
                type: 'hidden',
                name: 'pwa_id'
            }).putValue(document.get.pwa_id));
            if (document.get.pwa_device) {
                form.insert(new Element('input', {
                    type: 'hidden',
                    name: 'pwa_device'
                }).putValue(document.get.pwa_device));
            }
        }
    },
    ...
    countTotal: function (prices) {
        ...
        $H(prices).each(function (pair) {
            ...
            if (formProductInput && !formProductInput.checked) { return; }
   ...
   */
    ...
    initOtherV2Options: function () {
        var handleInputView = function (params) {
            ...
            input.addEventListener("keydown" , function(e){
              ...
              var otherInputOption = container.parentNode.querySelector('input[value="other"]');
    ...
    handleFormCollapse: function () {
        ...
        var openBar = false;
        var openCount = 0;
        var height = JotForm.newDefaultTheme ? 84 : 60;
    ...
    handleFITBInputs: function () {
        ...
        var fitbInputs = document.querySelectorAll('.FITB input[type="text"]');
        if (fitbInputs && fitbInputs.length > 0) {
            Array.from(fitbInputs).forEach(function(fitbInput) {
                var initWidth = fitbInput.offsetWidth;
                var fitbInputLabel = fitbInput.nextSibling;
                ...
                initWidth = Math.max(initWidth, width) + 8;
        ...
        if (fitbSignatures && fitbSignatures.length > 0) {
            Array.from(fitbSignatures).forEach(function(inputContainer) {
                ...
                if (signatureImage && signatureInput && signatureTrigger && window.JFFormSignature) {
                    ...
                    window.JFFormSignature({ trigger: signatureTrigger, onUse: onUse, getInitialValue: getInitialValue });
                }
            });
        }
    },
                    ...
                    try {
                        ...
                        var checkValues = inputs.map(function (e) {
                            ...
                            if (innerVals.include('required') && JotForm.isVisible(e)) {
                                if (JotForm.getContainer(e).getAttribute('data-type') === 'control_inline' && e.readAttribute('type') == "checkbox") {
                                    var baseName = e.name.substr(0, e.name.indexOf('-')); // name is like q{qid}_{qname}[{fieldID}-{blotIndex}]
        ...
        if (question && question.value) {
          ...
          switch (question.type) {
            case "control_fullname":
              var first = $("first_" + qid);
              var last = $("last_" + qid);
              var middle = $("middle_" + qid);
              var names = value.split(' ');
              if (names.length > 1) {
                last.value = names[names.length - 1];
                if (names.length >= 2) {
                  if (middle) {
                    middle.value = value.substring(value.indexOf(" "), value.lastIndexOf(" "));
                  } else {
                    first.value = value.substring(0, value.lastIndexOf(" "));
                  }
                }
              }
            break;
